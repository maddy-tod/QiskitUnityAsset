...
def terra_move(board):
    """
    Args:
        board [int] : the current state of the board
    Returns:
        (int, dict) : (the index of the move to be made, the t counts)
    """
    ...
    # None comes through as -1
    board = [None if int(space) == -1 else int(space) for space in board]
    ...
    print("Made the circuit, running it on the backend")
